...
# Cola simple por servidor
queues = {}        # guild_id: list of queries
now_playing = {}   # guild_id: title

# Opciones optimizadas para que NO se quede colgado
YTDL_OPTS = {
...
async def play_next(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="▶️ Reproduciendo ahora", description=title, color=0x1DB954)
        await ctx.send(embed=embed)

    except Exception as e:
        print(f"Error reproduciendo '{query}': {e}")
        await ctx.send(f"❌ No se pudo reproducir **{query}**. Inténtalo de nuevo.", delete_after=15)
        await play_next(ctx)


@bot.hybrid_command(name="play", description="Reproduce una canción")
async def play(ctx, *, query: str):
    """Comando principal - muy simple y rápido"""
...
async def on_ready():
    await bot.tree.sync()
    print(f"✅ Bot listo como {bot.user} - Usa !play o /play")
